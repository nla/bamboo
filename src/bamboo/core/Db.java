...
public abstract class Db implements AutoCloseable, Transactional {
	...
	public abstract List<Collection> listCollections();
	...
	@GetGeneratedKeys
	public abstract long createCollection(@BindBean Collection collection);
	...
	public abstract int updateCollection(@Bind("id") long collectionId, @BindBean("coll") Collection coll);
	...
	public static class CrawlWithSeriesNameMapper implements ResultSetMapper<CrawlAndSeriesName> {
		@Override
	...
	@GetGeneratedKeys
	public abstract long createPandasCrawlInternal(@Bind("name") String name, @Bind("crawl_series_id") Long crawlSeriesId, @Bind("state") int state, @Bind("pandasInstanceId") long pandasInstanceId);


	@Transaction
	public long createCrawlWithWarcs(Crawl crawl, List<Warc> warcs) {
		long totalBytes = warcs.stream().mapToLong(Warc::getSize).sum();
		long crawlId = createCrawl(crawl);
		batchInsertWarcsWithoutRollup(crawlId, warcs.iterator());
		incrementWarcStatsForCrawl(crawlId, warcs.size(), totalBytes);
		return crawlId;
	}
	...
	@GetGeneratedKeys
	public abstract long createCrawl(@BindBean Crawl crawl);
	...
	public abstract List<Crawl> findCrawlsByCrawlSeriesId(@Bind("crawl_series_id") long crawlSeriesId);
	...
	public abstract List<CrawlAndSeriesName> paginateCrawlsWithSeriesName(@Bind("limit") long limit, @Bind("offset") long offset);
	...
	public static class CrawlSeriesWithCount extends Series {
	...
	public static class CrawlSeriesMapper implements ResultSetMapper<Series> {
		@Override
	...
	public abstract Series findCrawlSeriesById(@Bind("id") long crawlSeriesId);

	@SqlQuery("SELECT * FROM crawl_series ORDER BY name")
	public abstract List<Series> listCrawlSeries();

	@SqlQuery("SELECT * FROM crawl_series WHERE path IS NOT NULL ORDER BY name")
	public abstract List<Series> listImportableCrawlSeries();
	...
	@SqlBatch("INSERT INTO seed (seedlist_id, url, surt) VALUES (:seedlistId, :url, :surt)")
	public abstract void insertSeedsOnly(@Bind("seedlistId") long seedlistId, @BindBean List<Seed> seeds);
	...
	@GetGeneratedKeys
	public abstract long createSeedlist(@BindBean Seedlist seedlist);
	...
	public abstract int updateSeedlist(@Bind("id") long id, @BindBean("seedlist") Seedlist seedlist, @Bind("totalSeeds") long totalSeeds);
	...
	@Transaction
	public void insertSeeds(long seedlistId, List<Seed> seeds) {
		insertSeedsOnly(seedlistId, seeds);
		incrementSeedlistTotalSeeds(seedlistId, seeds.size());
	}

	@Transaction
	public void updateSeedlist(long seedlistId, Seedlist seedlist, List<Seed> seeds) {
		deleteSeedsBySeedlistId(seedlistId);
		insertSeedsOnly(seedlistId, seeds);
		updateSeedlist(seedlistId, seedlist, seeds.size());
	}
	...
	public interface InsertableWarc {
		long getStateId();
		String getPath();
		String getFilename();
		long getSize();
	...
	public void incrementWarcStatsForCrawl(long crawlId, int warcFilesDelta, long sizeDelta) {
	...
	public abstract void incrementWarcStatsForCrawlInternal(@Bind("crawlId") long crawlId, @Bind("warc_files") int warcFilesDelta, @Bind("warc_size") long warcSizeDelta);
	...
	public abstract void batchInsertWarcsWithoutRollup(@Bind("crawlId") long crawlId, @BindBean("warc") Iterator<Warc> warcs);
	...
	public abstract long countWarcsWithCrawlId(@Bind("crawlId") long crawlId);
